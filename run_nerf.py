...
def render_rays(ray_batch, network_fn, network_query_fn, N_samples, retraw=False, lindisp=False, perturb=0.,
                N_importance=0, network_fine=None, white_bkgd=False, raw_noise_std=0., verbose=False):
    def raw2outputs(raw, z_vals, rays_d):
        ...
        disp_map = 1. / tf.maximum(1e-10, depth_map / tf.reduce_sum(input_tensor=weights, axis=-1))
    ...
    for i in range(start, N_iters):
        ...
        if i % args.i_print == 0 or i < 10:
            ...
            with tf.compat.v2.summary.record_if(lambda: tf.math.equal(0, global_step % args.i_print)):
                ...
                if args.N_importance > 0:
                    tf.compat.v2.summary.scalar(name='psnr0', data=psnr0,
                                                step=tf.compat.v1.train.get_or_create_global_step())
            ...
            if i % args.i_img == 0:
                ...
                with tf.compat.v2.summary.record_if(lambda: tf.math.equal(0, global_step % args.i_img)):
                    ...
                    tf.compat.v2.summary.image(name='rgb', data=to8b(rgb)[tf.newaxis],
                                               step=tf.compat.v1.train.get_or_create_global_step())
                    tf.compat.v2.summary.image(name='disp', data=disp[tf.newaxis, ..., tf.newaxis],
                                               step=tf.compat.v1.train.get_or_create_global_step())
                    tf.compat.v2.summary.image(name='acc', data=acc[tf.newaxis, ..., tf.newaxis],
                                               step=tf.compat.v1.train.get_or_create_global_step())

                    tf.compat.v2.summary.scalar(name='psnr_holdout', data=psnr,
                                                step=tf.compat.v1.train.get_or_create_global_step())
                    tf.compat.v2.summary.image(name='rgb_holdout', data=target[tf.newaxis],
                                               step=tf.compat.v1.train.get_or_create_global_step())
                ...
                if args.N_importance > 0:
                    with tf.compat.v2.summary.record_if(lambda: tf.math.equal(0, global_step % args.N_importance)):
                        ...
                        tf.compat.v2.summary.image(name='disp0', data=extras['disp0'][tf.newaxis, ..., tf.newaxis],
                                                   step=tf.compat.v1.train.get_or_create_global_step())
                        tf.compat.v2.summary.image(name='z_std', data=extras['z_std'][tf.newaxis, ..., tf.newaxis],
                                                   step=tf.compat.v1.train.get_or_create_global_step())
